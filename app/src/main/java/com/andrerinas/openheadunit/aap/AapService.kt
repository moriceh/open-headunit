        ...
        when (intent?.action) {
            ...
            ACTION_BT_AUTO_START          -> {
                ...
                if (settings.wifiConnectionMode == WifiLauncherMode.BLINK && !sessionUp) {
                    // Our unit-specific path: on a Choiceway/Blink unit the BT arrival means the
                    // hotspot must be up and the ZXW bridge re-armed, which upstream's policy has
                    // no knowledge of. It runs first and skips the generic re-arm decision below.
